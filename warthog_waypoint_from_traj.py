    ...
    for i in range(0, num_points - 1):
        ...
        while len(waypoint_list) < num_traj_point:
            ...
            if way_idx < len(df.index):
                if k == 1:
                    waypoint0 = np.array(
                        [df.iloc[way_idx]["x"], df.iloc[way_idx]["y"]])
                    waypoint_list.append(get_pose_from_df(df.iloc[way_idx]))
                    way_pose = get_pose_from_df(df.iloc[way_idx])
                    out_file_h.writelines(
                        f"{way_pose[0]}, {way_pose[1]}, {way_pose[2]}, {way_pose[3]}, {way_pose[4]}\n"
                    )
                waypoint1 = np.array(
                    [df.iloc[way_idx]["x"], df.iloc[way_idx]["y"]])
        ...
        for i in waypoint_list:
            warthog_env.waypoints_list.append(
                np.array([i[0], i[1], i[2], i[3]]))
