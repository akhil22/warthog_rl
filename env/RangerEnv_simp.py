...
class RangerEnv(gym.Env):
    ...
    def sim_ranger_simple(self, vel, brake, steer):
        x = self.pose[0]
        y = self.pose[1]
        th = self.pose[2]
        v_ = self.twist[0]
        w_ = self.twist[1]
        self.twist[0] = vel * 15
        self.twist[1] = 15 * vel * math.tan(
            steer * 45 * math.pi / 180) / self.ranger_wheel_base
    ...
    def step(self, action):
        ...
        if action[0] < 0:
            ...
            self.sim_ranger_simple(action[0], 0., action[1])
        ...
        #        math.pi / 3. - math.fabs(self.phi_error)) - math.fabs(
        #            self.accel_cmd[-1] -
        #            self.accel_cmd[-2]) - 2 * math.fabs(self.steer_cmd[-1])
        self.reward = (2.0 - math.fabs(self.crosstrack_error)) * (
            4.5 - math.fabs(self.vel_error)) - math.fabs(
                self.accel_cmd[-1] -
                self.accel_cmd[-2]) - 2 * math.fabs(self.steer_cmd[-1])
        #self.reward = (2.0 - math.fabs(self.crosstrack_error)) * (
