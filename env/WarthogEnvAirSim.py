...
class WarthogEnv(gym.Env):
    ...
    def sim_warthog(self, v, w):
        ...
        y = self.war_state.kinematics_estimated.orientation.y_val
        z = self.war_state.kinematics_estimated.orientation.z_val
        q = Quaternion(w, x, y, z)
        rot_axis = q.axis
        yaw = q.radians
        if rot_axis[2] >= 0:
            yaw = -1 * yaw
        self.pose[0] = self.war_state.kinematics_estimated.position.x_val
        self.pose[1] = -1 * self.war_state.kinematics_estimated.position.y_val
        self.pose[2] = yaw
