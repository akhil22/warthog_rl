...
with model.sess as sess:
    ...
    target = tf.Variable([1.0], dtype = tf.float32)
    target = np.array([[1]])
   ...
   # def1 = tf.multiply(model.act_model.deterministic_action, x)
    def1 = tf.multiply(model.act_model.deterministic_action, target_ph)
    ...
    action = model.sess.run(def1,{model.act_model.obs_ph: obs, target_ph: target})
